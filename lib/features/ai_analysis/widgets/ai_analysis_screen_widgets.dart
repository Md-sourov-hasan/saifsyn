      ...
      children: [
        ...
        Flex(
          ...
          children: [
            ...
            SizedBox(width: isCompact ? 0 : 12.w, height: isCompact ? 12.h : 0),
...
Widget _searchField({required AnalysisController controller}) {
...
Widget _languageToggle({required AnalysisController controller}) {
  return Obx(() {
    return Container(
      height: 52.h,
...
Widget _resultHeader({
  required CompanyAnalysisResult result,
...
Widget _snapshotCard({
  required StockSnapshot snapshot,
...
Widget _chartCard({
  ...
  final minY =
      points.isEmpty ? 0.0 : points.map((e) => e.close).reduce(math.min) * 0.96;
  final maxY =
      points.isEmpty ? 1.0 : points.map((e) => e.close).reduce(math.max) * 1.04;
...
Widget _sectionCard({
  required AnalysisSection section,
  required bool isMobile,
}) {
  return Container(
    padding: EdgeInsets.all(isMobile ? 16.w : 20.w),
...
Widget _analysisTableWidget({required AnalysisTable table}) {
...
Widget _metaPill({required String label}) {
  return Container(
...
Widget _inlineError({required String message}) {
  return Container(
    width: double.infinity,
    padding: EdgeInsets.symmetric(horizontal: 14.w, vertical: 12.h),
    decoration: BoxDecoration(
